...
const getFormSchema = (t, item) => {
  const formSchema = yup.object().shape({
    name: yup
      ...
      .min(4, t("validations.min_length_4")),
  });
...
const TagCreateEdit = ({ item, ...props }) => {
  ...
  const onSubmit = async data => {
    ...
    const url = item
      ? process.env.REACT_APP_BACKEND_URL + "/tags/" + item.id
  ...
  return (
    <div className="entity-new">
      {!httpClientHook.error && requestSend && !httpClientHook.isLoading ? (
        <AlertSuccess
          title={t("tag_create_edit_form.alert_success_title")}
          description={t("tag_create_edit_form.alert_success_description")}
          ...
          <div className="row">
            <div className="column full-witdh">
              <Input
                id="name"
                type="text"
                label={t("tag_create_edit_form.input_name_label")}
